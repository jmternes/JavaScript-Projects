...
const Calculator = {
    ...
    Display_Value: '0',
    //this will hold the first operand for any expressions, null for now
    First_Operand: null,
...
function Input_Digit(digit) {
    ...
    //we are seeing if Wait_Second_Operand is true and set
...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //we are saying that if the display value doesnt contain a decimal
...
keys.addEventListener('click', (event) => {
    //the target variable is an object that represents the element clicked
    const { target } = event;
    //if the element clicked on is not a butotn, exit the function
